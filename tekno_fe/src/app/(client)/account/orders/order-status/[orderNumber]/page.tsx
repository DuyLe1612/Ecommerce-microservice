...
import { ArrowLeft, Check, Clock, Package, Star, Truck } from "lucide-react";
    ...
    <div className="max-w-5xl mx-auto p-4 md:p-6 space-y-6 text-white">
      ...
      <div className="rounded-lg border border-white/10 bg-white/[0.03] p-5 md:p-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-8">
          <div>
            ...
            <div className={isCancelled ? "text-lg font-semibold text-red-400" : delivered ? "text-lg font-semibold text-green-400" : "text-lg font-semibold text-primary"}>
          ...
          {TRACKING_STEPS.map((step, index) => {
            ...
            return (
              <div key={step.key} className="flex flex-col items-center text-center gap-2">
                <div className={`w-11 h-11 rounded-full flex items-center justify-center border ${active ? "bg-primary text-black border-primary" : "bg-white/5 text-white/35 border-white/10"
                  }`}>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          {delivered && (
            <span className="text-xs text-green-400 bg-green-400/10 border border-green-400/20 px-2.5 py-1 rounded-full font-medium">
              ✅ Delivered — you can review these products
            </span>
          )}
        </div>
        ...
        {items.map((item) => {
          ...
          return (
            <div
              ...
              className="flex items-center gap-4 border border-white/10 rounded-lg p-3 bg-white/[0.03] hover:border-white/20 transition-colors"
            >
              <Link href={`/products/${item.productSlug ?? item.product?.slug ?? generateProductSlug(title, item.productId)}`} className="relative w-20 h-20 shrink-0 rounded-md overflow-hidden bg-white/5 border border-white/10 hover:border-primary/50 transition-colors block">
                {image ? (
              ...
              <div className="flex-1 min-w-0">
                <Link href={`/products/${item.productSlug ?? item.product?.slug ?? generateProductSlug(title, item.productId)}`} className="text-sm font-medium line-clamp-2 hover:text-primary transition-colors block">{title}</Link>
              ...
              <div className="flex flex-col items-end gap-2">
                ...
                {delivered && (
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full border border-primary/40 text-primary hover:bg-primary/10 hover:border-primary/70 transition-all active:scale-95 whitespace-nowrap"
                  >
